<div class="form-group {{ $errors->has('Marca') ? 'has-error' : ''}}">
    ...
    <div class="col-md-6">
        {!! Form::text('Marca', null, ['class' => 'form-control','autofocus'=>'autofocus', 'id'=>'Marca']), old('Marca')  !!}
    ...
    @foreach (json_decode('{"1":"Activo","0":"Inactivo"}', true) as $optionKey => $optionValue)
        <option value="{{ $optionKey }}" {{ (isset($marca->activo) && $marca->activo == $optionKey) ? 'selected' : ''}}>{{ $optionValue }}</option>
    @endforeach
</select>
